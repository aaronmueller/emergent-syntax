...
class ModelArguments:
    ...
    from_flax: bool = field(
        default=False,
        metadata={
            "help": "Whether model was trained using flax."
        },
    )
...
def main():
    ...
    tokenizer = (T5Tokenizer if 't5-efficient' in model_args.model_name_or_path
    ...
    if training_args.do_eval:
        ...
        if "passiv" in data_args.validation_file:
            word_idx = "Second"
        ...
        if trainer.is_world_process_zero():
            with open(output_eval_file, "w") as writer:
                ...
                writer.write(f"{word_idx} word accuracy: {first_acc}\n")
    ...
    if data_args.do_learning_curve:
      ...
      for path in glob.glob(os.path.join(training_args.output_dir, "checkpoint-*", "")):
        ...
        if trainer.is_world_process_zero():
            with open(output_eval_file, "w") as writer:
              writer.write(f"iteration,exact_match,{word_idx}_word_accuracy\n")
      ...
      plt.plot(its,first_accs, label=f"{word_idx} word accuracy")
      plt.legend()
